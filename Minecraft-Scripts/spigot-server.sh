...
command -v git >/dev/null 2>&1 || { echo -e >&2 "$ERROR You most have git installed on your system for this script to work.  How to install: https://git-scm.com/book/en/v2/Getting-Started-Installing-Git"; exit 1; }
...
if [ -d "BuildTools" ]; then
    echo -e "$ERROR The BuildTools directory already exists."
    echo -e "$ERROR Please delete the BuildTools directory and then start this script again."
    exit 1
fi
# Makes the BuildTools directory
echo -e "$INFO Making BuildTools directory."; mkdir BuildTools
# Downloads BuildTools
echo -e "$INFO Downloading BuildTools..."; wget https://hub.spigotmc.org/jenkins/job/BuildTools/lastSuccessfulBuild/artifact/target/BuildTools.jar &> /dev/null; echo -e "$INFO Finished downloading BuildTools!"
# Moves the BuildTools jar to the BuildTools directory
echo -e "$INFO Moving jar file to BuildTools directory."; mv BuildTools.jar ./BuildTools/
# Enters BuildTools directory
echo -e "$INFO Entering BuildTools directory"; cd BuildTools
# Builds the provided Spigot version
echo -e "$INFO Building Spigot version $version... This may take a while."; sleep 5; java -jar BuildTools.jar --rev $version; echo -e "$INFO Done building Spigot jar!"
# Moves back to the directory of where the script is located
echo -e "$INFO Moving back to base directory."; cd ..
# Check to see if the provided server directory already exists
if [ -d "$server_directory" ]; then
    echo -e "$ERROR The provided server directory already exists."
    echo -e "$ERROR Please provide another server directory or delete the provided directory."
    exit 1
fi
# Makes server directory
echo -e "$INFO Making server directory."; mkdir $server_directory
# Moves spigot jar to server directory
echo -e "$INFO Moving the Spigot jar to the server directory."; mv ./BuildTools/spigot-$version.jar ./$server_directory/
# Deletes the BuildTools directory
echo -e "$INFO Deleting the BuildTools directory."; rm -rf BuildTools
...
# Starts the server
echo -e "$INFO Starting screen session!"; screen -S minecraft -dm java -Xms2G -Xmx2G -jar spigot-$version.jar; echo -e "$INFO You can see the server console with screen -r minecraft"